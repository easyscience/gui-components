...
function bokehHead(specs) {
  const list = [headCommon(), bokehHeadScripts(), bokehHeadStyle(specs)]
  return list.join('\n')
}
...
function bokehAddMainTools(chart) {
  return [
    `${chart}.add_tools(new Bokeh.HoverTool({tooltips:main_tooltip, point_policy:"snap_to_data", mode:"mouse"}))`,
...
function bokehAddVisibleXAxis(chart, specs) {
  return [
    `${chart}.xaxis[0].axis_label_text_font = "PT Sans"`,
...
function bokehAddHiddenXAxis(chart, specs) {
  return [
    `${chart}.xaxis[0].axis_label_text_font_size = "0px"`,
    `${chart}.xaxis[0].axis_line_color = null`,
...
function bokehAddVisibleYAxis(chart, specs) {
  return [
    `${chart}.yaxis[0].axis_label_text_font = "PT Sans"`,
...
function bokehAddBackgroundDataToMainChart(data, specs) {
  return [
    `main_source.data.x_bkg = [${data.background.x}]`,
...
function bokehAddMeasuredDataToMainChart(data, specs) {
  return [
    `main_source.data.x_meas = [${data.measured.x}]`,
...
function bokehAddCalculatedDataToMainChart(data, specs) {
  return [
    `main_source.data.x_calc = [${data.calculated.x}]`,
...
function bokehAddPhaseDataToMainChart(data, specs) {
  ...
  for (const phase_index in data.phase) {
    out.push(
      `main_source.data.x_phase_${phase_index} = [${data.phase[phase_index].x}]`,
    )
    out.push(
      `main_source.data.y_phase_${phase_index}_upper = [${data.phase[phase_index].y_upper}]`,
    )
    out.push(
      `main_source.data.y_phase_${phase_index}_lower = [${data.phase[phase_index].y_lower}]`,
    )
...
function bokehAddDataToBraggChart(data, specs) {
  ...
  for (const phase_index in data.bragg) {
    ...
    out.push(
      `const bragg_source_${phase_index} = new Bokeh.ColumnDataSource()`,
    )

    out.push(
      `bragg_source_${phase_index}.data.x_bragg = [${data.bragg[phase_index].x}]`,
    )
    out.push(
      `bragg_source_${phase_index}.data.y_bragg = [${data.bragg[phase_index].y}]`,
    )
    out.push(
      `bragg_source_${phase_index}.data.h_bragg = [${data.bragg[phase_index].h}]`,
    )
    out.push(
      `bragg_source_${phase_index}.data.k_bragg = [${data.bragg[phase_index].k}]`,
    )
    out.push(
      `bragg_source_${phase_index}.data.l_bragg = [${data.bragg[phase_index].l}]`,
    )
    ...
    out.push('})')
    out.push(
      `bragg_chart.add_glyph(braggTicks_${phase_index}, bragg_source_${phase_index})`,
    )
  }
  return out
}
...
function bokehAddDataToDiffChart(data, specs) {
  return [
    `main_source.data.x_diff = [${data.difference.x}]`,
...
function bokehAddMainTooltip(data, specs) {
  const x_meas = bokehMainTooltipRow(
    EaStyle.Colors.themeForegroundDisabled,
    'x',
    '@x_meas{0.00}',
  )
  const x_calc = bokehMainTooltipRow(
    EaStyle.Colors.themeForegroundDisabled,
    'x',
    '@x_calc{0.00}',
  )
  const y_meas = bokehMainTooltipRow(
    specs.measuredLineColor,
    'meas',
    '@y_meas{0.0}',
    '&#177;&nbsp;@sy_meas{0.0}',
  )
  const y_calc = bokehMainTooltipRow(
    specs.calculatedLineColor,
    'calc',
    '@y_calc{0.0}',
  )
  const y_bkg = bokehMainTooltipRow(
    specs.backgroundLineColor,
    'bkg',
    '@y_bkg{0.0}',
  )
  const y_diff = bokehMainTooltipRow(
    specs.differenceLineColor,
    'diff',
    '@y_diff{0.0}',
  )
  let y_phases = []
  for (const phase_index in data.phase) {
    if (typeof specs.phaseLineColor !== 'undefined') {
      y_phases.push(
        bokehMainTooltipRow(
          specs.phaseLineColor[phase_index],
          `phase ${phase_index}`,
          `@y_phase_${phase_index}_upper{0.0}`,
        ),
      )
    }
  }
...
function bokehAddBraggTooltip(specs) {
  const x_bragg = bokehBraggTooltipSpan(
    EaStyle.Colors.themeForegroundDisabled,
    'x',
    '@x_bragg{0.00}',
  )
  const hkl_bragg = bokehBraggTooltipSpan(
    specs.calculatedLineColor,
    'hkl',
    '(@h_bragg @k_bragg @l_bragg)',
  )
